...
if(isset($_POST['csv'])) {
        ...
        foreach($csv_strings as $str) {
                ...
                $bitcoins_balance=db_query_to_variable("SELECT `value` FROM `results` WHERE `id`='$id_escaped'");
                $balance_diff=$bitcoin-$bitcoins_balance;
                if($balance_diff>0) {
                        echo "New $balance_diff BTC, add to $user_uid\n";
                        update_user_results($user_uid,"Freebitcoin",$bitcoin);
                } else {
